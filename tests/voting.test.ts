...
const ensureEncounter = async () => {
  const { data: current } = await api("GET", "/api/rencontre/current", undefined, ADMIN);
  if (!current?.id) {
    await api("POST", `/api/rencontre/start/${ENCOUNTER_ID}`, undefined, ADMIN);
  }
};

const getRemaining = async (): Promise<any[]> => {
  const { data } = await api("GET", "/api/syndicat/remaining", undefined, ADMIN);
  return data;
};

const getCurrentVote = async (): Promise<any> => {
  const { data } = await api("GET", "/api/vote/current", undefined, ADMIN);
  return data;
};

const createVote = async (opts: {
  nom: string;
  type?: string;
  texteId?: number;
  possibilites?: string[];
}) => {
  const { data } = await api("POST", "/api/vote", {
    nom: opts.nom,
    description: null,
...
const startVote = (id: number) =>
  api("POST", `/api/vote/start/${id}`, undefined, ADMIN);

const stopVote = () => api("POST", "/api/vote/stop", undefined, ADMIN);

const castVote = async (
  choix: { type: string | number; mandat: number }[],
  syndicatName?: string,
) => {
  const syndicat = syndicatName
    ? (await api("GET", `/api/syndicat/${syndicatName}`, undefined, ADMIN)).data
    : undefined;
  const body: any = { choix };
...
const toggleResults = (voteId: number) =>
  api("POST", "/api/vote/toggle-results", { voteId }, ADMIN);
...
describe("Vote lifecycle", () => {
  it("create → start → vote → stop", async () => {
    const vote: any = await createVote({ nom: "Lifecycle Test" });
...
describe("STANDARD vote", () => {
  it("allows voting POUR with correct mandats", async () => {
    const vote: any = await createVote({ nom: "Standard POUR" });
    ...
    const current = await getCurrentVote();
    const bordeauxChoix = current.choix.find((c: any) => c.syndicat.nom === "Bordeaux");
  ...
  it("allows voting CONTRE", async () => {
    const vote: any = await createVote({ nom: "Standard CONTRE" });
    await startVote(vote.id);

    await castVote([{ type: "CONTRE", mandat: 1 }], "Bordeaux");

    const current = await getCurrentVote();
    const choix = current.choix.find((c: any) => c.syndicat.nom === "Bordeaux");
    expect(choix.choix[0].type).toBe("CONTRE");
  });

  it("allows voting ABSTENTION", async () => {
    const vote: any = await createVote({ nom: "Standard ABST" });
    await startVote(vote.id);
  ...
  it("overwrites a previous vote from the same syndicat", async () => {
    const vote: any = await createVote({ nom: "Overwrite test" });
    await startVote(vote.id);
  ...
  it("allows multiple syndicats to vote independently", async () => {
    const vote: any = await createVote({ nom: "Multi syndicat" });
    await startVote(vote.id);
...
describe("Panachage", () => {
  it("splits mandats evenly across two choices", async () => {
    const vote: any = await createVote({ nom: "Panachage even" });
    await startVote(vote.id);
    ...
    const current = await getCurrentVote();
    const choix = current.choix.find((c: any) => c.syndicat.nom === "Nantes");
    expect(choix.choix.length).toBe(2);
    expect(choix.choix.find((e: any) => e.type === "POUR").mandat).toBe(1);
    expect(choix.choix.find((e: any) => e.type === "CONTRE").mandat).toBe(1);
  });

  it("splits mandats unevenly (2+1 for 3-mandat syndicat)", async () => {
    const vote: any = await createVote({ nom: "Panachage uneven" });
    await startVote(vote.id);
    ...
    const current = await getCurrentVote();
    const choix = current.choix.find((c: any) => c.syndicat.nom === "Paris");
    expect(choix.choix.length).toBe(2);
    expect(choix.choix.find((e: any) => e.type === "POUR").mandat).toBe(2);
    expect(choix.choix.find((e: any) => e.type === "CONTRE").mandat).toBe(1);
  });

  it("puts all mandats on a single choice", async () => {
    const vote: any = await createVote({ nom: "Panachage single" });
    await startVote(vote.id);

    await castVote([{ type: "POUR", mandat: 3 }], "Lyon");

    const current = await getCurrentVote();
    const choix = current.choix.find((c: any) => c.syndicat.nom === "Lyon");
  ...
  it("splits across three choices (1+1+1 for 3-mandat syndicat)", async () => {
    const vote: any = await createVote({ nom: "Panachage triple" });
    await startVote(vote.id);
    ...
    expect(choix.choix.find((e: any) => e.type === "POUR").mandat).toBe(1);
    expect(choix.choix.find((e: any) => e.type === "CONTRE").mandat).toBe(1);
    expect(choix.choix.find((e: any) => e.type === "ABSTENTION").mandat).toBe(1);
  });

  it("rejects when mandat sum is too high", async () => {
    const vote: any = await createVote({ nom: "Bad panachage high" });
    await startVote(vote.id);
  ...
  it("rejects when mandat sum is too low", async () => {
    const vote: any = await createVote({ nom: "Bad panachage low" });
    await startVote(vote.id);
...
describe("Hide results", () => {
  it("toggles hideResults flag", async () => {
    const vote: any = await createVote({ nom: "Hide results test" });
  ...
  it("resets hideResults when vote is stopped", async () => {
    const vote: any = await createVote({ nom: "Reset on stop" });
    await startVote(vote.id);
  ...
  it("rejects non-admin toggle", async () => {
    const vote: any = await createVote({ nom: "Admin only toggle" });
    await startVote(vote.id);
...
describe("Syndicats remaining", () => {
  it("decreases as syndicats vote", async () => {
    const vote: any = await createVote({ nom: "Remaining test" });
    await startVote(vote.id);
    ...
    expect(after.find((s: any) => s.nom === "Bordeaux")).toBeUndefined();
  });

  it("returns empty when all syndicats have voted", async () => {
    const vote: any = await createVote({ nom: "All voted" });
    await startVote(vote.id);

    const before = await getRemaining();
    for (const s of before) {
      const totalMandats = s.mandats.reduce((sum: number, m: any) => sum + m.mandat, 0);
...
describe("Error cases", () => {
  it("rejects vote with wrong mandat count", async () => {
    const vote: any = await createVote({ nom: "Wrong mandats" });
    await startVote(vote.id);
  ...
  it("rejects starting a vote when one is already active", async () => {
    const v1: any = await createVote({ nom: "Vote A" });
    const v2: any = await createVote({ nom: "Vote B" });
  ...
  it("rejects vote with invalid choice type", async () => {
    const vote: any = await createVote({ nom: "Bad type" });
    await startVote(vote.id);
  ...
  it("rejects non-admin from starting vote", async () => {
    const vote: any = await createVote({ nom: "Admin start only" });
  ...
  it("rejects vote for unknown syndicat name", async () => {
    const vote: any = await createVote({ nom: "Unknown syndicat" });
    await startVote(vote.id);
...
describe("CONDORCET vote", () => {
  it("creates with possibilites", async () => {
    const vote: any = await createVote({
    ...
    expect(vote.possibilites.map((p: any) => p.nom).sort()).toEqual(["Chat", "Chien", "Poisson"]);
  });
  ...
  it("accepts a ranking vote", async () => {
    ...
    const ids = vote.possibilites.map((p: any) => p.id);
...
describe("Condorcet Panachage", () => {
  it("splits mandats across two different rankings", async () => {
    ...
    expect(choix.choix[0].vote).toEqual([ids[0], ids[1], ids[2]]);
    expect(choix.choix[1].mandat).toBe(1);
    expect(choix.choix[1].vote).toEqual([ids[1], ids[2], ids[0]]);
  });
...
describe("Condorcet result calculation", () => {
  it("identifies a clear winner (A beats B and C)", async () => {
    ...
    const current = await getCurrentVote();
    const choiceMeta: CondorcetChoiceMeta[] = current.possibilites.map((p: any) => ({
      key: p.id,
      label: p.nom,
    }));

    const { matrix, winnerIndex, winnerLabel } = condorcetResult(current.choix, choiceMeta);
  ...
  it("detects a Condorcet paradox (A>B>C>A cycle)", async () => {
    ...
    const { matrix, winnerIndex, isParadox } = condorcetResult(current.choix, choiceMeta);
  ...
  it("handles panachage in result calculation", async () => {
    ...
    const { winnerIndex, winnerLabel } = condorcetResult(current.choix, choiceMeta);
  ...
  it("reports matrix values correctly for equal opposing votes", async () => {
    ...
    const { matrix, winnerIndex } = condorcetResult(current.choix, choiceMeta);
